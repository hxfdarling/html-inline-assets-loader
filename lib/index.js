...
const { getLink, isLink, isStyle, isHtml } = require('./util');
      ...
      if (needInclude) {
        ...
        if (inline || isHtml(node)) {
          if (nodeName === SCRIPT) {
            node.attrs = [];
            node.childNodes = [
              { nodeName: '#text', value: result, parentNode: node },
            ];
            ...
            childNodes.splice(
              childNodes.indexOf(node), 1, ...htmlDom.childNodes);
          } else {
            const msg = `\nonly js/css support inline:${
              JSON.stringify(
                { tagName: node.tagName, attrs: node.attrs }, null, 2)}`;
          ...
          const newFileName = `${path.basename(file).split('.')[0]}_${hash}${
            path.extname(file)}`;
